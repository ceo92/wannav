...
> - 기본 예약금은 10,000원이고 변경을 원하면 식당 등록 시 예약금을 별도로 지정할 수 있음
- 예약 취소 하루 전부터 패널티 발생
...
> 고객은 식당 예약 후 식사를 하면 영수증을 발급받는다. 그 후 영수증 인증 리뷰를 작성하고, 리워드를 적립받으면 해당 리워드를 사용하여 상품을 구매할 수 있다.
...
- 그에 따라 다음과 같이 Restaurant 엔티티 내부에 정적 팩토리 메서드를 정의함
...
- Restaurant 엔티티 내부에 정적 팩토리 메서드```createRestaurant```를 정의하였지만 수많은 필드 및 연관관계를 설정해야함에 따라 객체 생성 시에 할당해야함에 따라 구조가 굉장히 복잡해짐
- 위와 같이 정의하니 일일이 값들을 설정해주는 작업이 굉장히 번거로웠고,**가장 치명적인 문제로 인자의 순서가 바뀌어도 모른다는 문제가 존재**
- 그에 따라 해당 패턴을 제거하고, Builder 패턴을 도입함
...
- 그렇다고 정적 팩토리 메서드 내부에 객체 생성에 대한 규격을 전부 정의하기엔 잘못된 순서의 인자 할당 문제가 발생함
- 해당 프로젝트에선 빌더 패턴으로 객체 생성을 해서 가장 치명적인 잘못된 순서의 인자 할당 문제를 잡되, 팀원들에게 Restaurant 엔티티에 대한 객체를 생성해야한다면 Restaurant과의 연관관계 설정에 대한 부분은 필수적으로 이루어져야한다고 주석과 공유 문서를 활용하여 안내하는 방법을 선택함
- 즉 빌더 패턴과 정적 팩토리 메서드 패턴 간의 trade - off 가 있으므로 상황에 따라 유연하게 선택해서 쓰면 된다는 결론을 내림
